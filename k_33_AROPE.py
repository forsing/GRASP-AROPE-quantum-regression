...
# AROPE (Asymmetric transitivity preserving)

# GRASP + kvantna regresija, deterministički, strukturno
# Varijanta: AROPE – asimetrična tranzitivnost (SVD kombinacije P i P^T)

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data.
AROPE (Asymmetric transitivity preserving): matrica S = P + α·P^T + viši redovi
zadržava asimetričnu tranzitivnost; SVD od S daje ugrađivanje.
"""
...
AROPE_ALPHA = 0.5 # 0.5 alpha for AROPE
AROPE_ORDERS = 3  # 3 orders for AROPE
...
def compute_arope_embeddings(M, k=EMBED_DIM):
    ...
    S = P + AROPE_ALPHA * P.T
    Pt = P.copy()
    for t in range(1, AROPE_ORDERS):
        Pt = Pt @ P
        S = S + (AROPE_ALPHA ** t) * (Pt + Pt.T)
...
def main():
    ...
    emb = compute_arope_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (AROPE + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (AROPE + kvantna regresija, deterministički, strukturno):
    (8, 11, x, y, z, 33, 34)

    Score: 0.5049613066920483
    """
...
Matrica asimetrične tranzitivnosti: 
S = P + α·Pᵀ + α·(P² + (P²)ᵀ) + α²·(P³ + (P³)ᵀ) + … 
(AROPE_ALPHA=0.5, AROPE_ORDERS=3).

SVD: S = U Σ Vᵀ, embedding = prvih k kolona od U√Σ.
